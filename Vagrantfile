...
Vagrant.configure(VAGRANT_API_VERSION) do |config|
  ...
  if ENV['ANSIBLE_NGINX_VAGRANT_BOXNAME']
    config.vm.box = ENV['ANSIBLE_NGINX_VAGRANT_BOXNAME']
  else
    config.vm.box = 'ubuntu/trusty64'
  end

  config.vm.define :ansiblenginxtest do |d|
    ...
    d.vm.provision :ansible do |ansible|
      ...
      ansible.skip_tags = ENV['ANSIBLE_NGINX_VAGRANT_ANSIBLE_SKIP_TAGS']
      ansible.verbose = ENV['ANSIBLE_NGINX_VAGRANT_ANSIBLE_VERBOSE']
    ...
    d.vm.provider :virtualbox do |v|
      ...
      v.customize [ 'modifyvm', :id, '--name', 'ansiblenginxtest', '--memory', '512', '--cpus', '1' ]
    end
